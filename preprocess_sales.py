...
# File Picker Helper
# -----------------------------
def _pick_excel_file(title: str) -> Optional[str]:
    """
    Opens a file dialog to pick an Excel file (.xlsx).
...
def add_ordered_rolling_features(
    ...
    # Lag1
    df["ordered_lag1"] = (
        df.groupby("product_id", observed=True)["ordered_qty"].shift(1).fillna(0.0)
    )
    ...
    for w in windows:
        df[f"ordered_roll_mean_{w}"] = gb.transform(
            lambda s: s.rolling(w, min_periods=1).mean()
        )
        df[f"ordered_roll_std_{w}"] = gb.transform(
    ...
    roll_cols = (
        ["ordered_lag1"]
        + [f"ordered_roll_mean_{w}" for w in windows]
        + [f"ordered_roll_std_{w}" for w in windows]
    )
...
def compute_basic_diagnostics(
    tx: pd.DataFrame, item_month: pd.DataFrame
) -> Dict[str, float]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to sales 2017.xlsx (if not provided, file picker will open)",
    )
    parser.add_argument(
        "--sales2018",
        type=str,
        required=False,
        default=None,
        help="Path to sales 2018.xlsx (if not provided, file picker will open)",
    )
    parser.add_argument(
        "--sales2019",
        type=str,
        required=False,
        default=None,
        help="Path to sales 2019.xlsx (if not provided, file picker will open)",
    )
    ...
    # Use file picker if paths not provided via command line
    sales2017_path = args.sales2017
    if not sales2017_path:
        LOG.info("Opening file picker for Sales 2017...")
        sales2017_path = _pick_excel_file("Επιλέξτε το αρχείο Sales 2017.xlsx")
        if not sales2017_path:
            LOG.error("No file selected for Sales 2017. Exiting.")
            return
        LOG.info("Selected: %s", sales2017_path)

    sales2018_path = args.sales2018
    if not sales2018_path:
        LOG.info("Opening file picker for Sales 2018...")
        sales2018_path = _pick_excel_file("Επιλέξτε το αρχείο Sales 2018.xlsx")
        if not sales2018_path:
            LOG.error("No file selected for Sales 2018. Exiting.")
            return
        LOG.info("Selected: %s", sales2018_path)

    sales2019_path = args.sales2019
    if not sales2019_path:
        LOG.info("Opening file picker for Sales 2019...")
        sales2019_path = _pick_excel_file("Επιλέξτε το αρχείο Sales 2019.xlsx")
        if not sales2019_path:
            LOG.error("No file selected for Sales 2019. Exiting.")
            return
        LOG.info("Selected: %s", sales2019_path)
    ...
    sales_paths = [
        (sales2017_path, args.sheet2017),
        (sales2018_path, args.sheet2018),
        (sales2019_path, args.sheet2019),
    ]
    ...
    tx.to_csv(tx_out, index=False, compression="gzip", float_format="%.3f")
    LOG.info("Wrote %s", tx_out)

    im_out = outdir / "item_month_agg_2017_2019.csv"
    item_month.to_csv(im_out, index=False, float_format="%.3f")
    LOG.info("Wrote %s", im_out)

    # Split on observed-only aggregation (optional, kept to match earlier artifacts)
    train_base, test_base = split_train_test(
        item_month, args.train_end, args.test_start
    )

    train_base_out = (
        outdir
        / f"train_item_month_2017_2019_cutoff_{pd.Timestamp(args.train_end).strftime('%Y-%m')}.csv"
    )
    test_base_out = (
        outdir
        / f"test_item_month_2017_2019_start_{pd.Timestamp(args.test_start).strftime('%Y-%m')}.csv"
    )
    train_base.to_csv(train_base_out, index=False, float_format="%.3f")
    test_base.to_csv(test_base_out, index=False, float_format="%.3f")
    ...
    panel_with_roll.to_csv(panel_out, index=False, float_format="%.3f")
    LOG.info("Wrote %s", panel_out)

    train_roll, test_roll = split_train_test(
        panel_with_roll, args.train_end, args.test_start
    )

    train_roll_out = (
        outdir
        / f"train_item_month_2017_2019_cutoff_{pd.Timestamp(args.train_end).strftime('%Y-%m')}_with_roll.csv"
    )
    test_roll_out = (
        outdir
        / f"test_item_month_2017_2019_start_{pd.Timestamp(args.test_start).strftime('%Y-%m')}_with_roll.csv"
    )
    train_roll.to_csv(train_roll_out, index=False, float_format="%.3f")
    test_roll.to_csv(test_roll_out, index=False, float_format="%.3f")
